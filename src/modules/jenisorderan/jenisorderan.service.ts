...
import { CreateJenisOrderanDto } from './dto/create-jenisorderan.dto';
import { UpdateJenisOrderanDto } from './dto/update-jenisorderan.dto';
...
export class JenisOrderanService {
  ...
  async findAll(
    ...
    try {
      ...
      if (isLookUp) {
        const jenisorderanCountResult = await trx(this.tableName)
          .count('id as total')
          .first();

        const jenisorderanCount = jenisorderanCountResult?.total || 0;
        if (Number(jenisorderanCount) > 500) {
      ...
      const query = trx(`${this.tableName} as jenisorderan`)
        .select([
          ...
          trx.raw(
            "FORMAT(jenisorderan.created_at, 'dd-MM-yyyy HH:mm:ss') as created_at",
          ),
          trx.raw(
            "FORMAT(jenisorderan.updated_at, 'dd-MM-yyyy HH:mm:ss') as updated_at",
          ),
          'par.memo',
          'par.text',
        ])
        .leftJoin('parameter as par', 'jenisorderan.statusaktif', 'par.id');
        ...
        for (const [key, value] of Object.entries(filters)) {
          ...
          if (value) {
            if (key === 'created_at' || key === 'updated_at') {
              ...
              query.andWhere(`jenisorderan.${key}`, 'like', `%${sanitizedValue}%`);
            }
          }
        }
      }
  ...
  async findAllByIds(ids: { id: number }[]) {
    try {
      ...
      const query = dbMssql(`${this.tableName} as jenisorderan`)
        .select([
          'jenisorderan.id as id',
          'jenisorderan.nama',
          'jenisorderan.keterangan',
        ...
        .leftJoin('parameter as par', 'jenisorderan.statusaktif', 'par.id')
        .join(dbMssql.raw(`${tempData} as temp`), 'jenisorderan.id', 'temp.id')

        .orderBy('jenisorderan.nama', 'ASC');
