...
export class GandenganService {
  ...
  async findAll(
    ...
    try {
      ...
      console.error('Error to findAll Gandengan in Service', error);
      throw new Error(error);
    }
  }

  findOne(id: number) {
    return `This action returns a #${id} gandengan`;
  }

  update(id: number, updateGandenganDto: UpdateGandenganDto) {
    return `This action updates a #${id} gandengan`;
  }

  remove(id: number) {
    return `This action removes a #${id} gandengan`;
  }
}
