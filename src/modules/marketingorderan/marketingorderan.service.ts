...
export class MarketingorderanService {
  ...
  async create(marketingOrderanData: any, marketing_id: any = 0, trx: any = null) {
    try {
      ...
      const fixData = marketingOrderanData.map(({ statusaktif_nama, ...marketingOrderanData }) => ({
      ...
      const getDeleted = await trx(this.tableName)
        .leftJoin(`${tempTableName}`, 'marketingorderan.id', `${tempTableName}.id`)
        .select(
          'marketingorderan.id',
      ...
      if (insertedDataQuery.length > 0) {
        insertedData = await trx('marketingorderan')
          ...
          .then((result: any) => result[0])
          .catch((error: any) => {
            console.error('Error inserting data marketing orderan in service:', error);
            throw error;
          });
      }
