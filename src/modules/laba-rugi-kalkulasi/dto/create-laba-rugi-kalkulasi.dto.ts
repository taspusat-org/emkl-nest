...
const baseFields = {
  periode: z
    .string()
    .nonempty({ message: 'PERIODE WAJIB DIISI' }),
...
export const CreateLabaRugiKalkulasiSchema = z
  ...
  .superRefine(async (data, ctx) => {
    ...
    const existsPeriode = await isRecordExist('periode', data.periode, 'labarugikalkulasi');
...
export type CreateLabaRugiKalkulasiDto = z.infer<typeof CreateLabaRugiKalkulasiSchema>;
