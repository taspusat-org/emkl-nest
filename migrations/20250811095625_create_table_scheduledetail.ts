...
export async function up(knex: Knex): Promise<void> {
    await knex.schema.createTable('scheduledetail', (table) => {
        ...
        table.dateTime('closing').nullable(); // closing (datetime)
        ...
        table.dateTime('created_at').defaultTo(knex.fn.now()); // created_at (datetime)
        table.dateTime('updated_at').defaultTo(knex.fn.now()); // updated_at (datetime)
    })
}
